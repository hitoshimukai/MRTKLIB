 ...
 */
static int chunk_decode(chunk_dec_t* dec, const uint8_t** pin, int* pnin, uint8_t* out, int nout) {
 ...
 */
static int chunk_encode(uint8_t* out, int nout, const uint8_t* data, int ndata) {
    char hdr[16];
...
static int http_status_code(const uint8_t* buff, int nb) {
    ...
    for (i = 5; i < nb; i++) {
        if (buff[i] == ' ') {
            ...
            if (buff[i + 1] >= '0' && buff[i + 1] <= '9' && buff[i + 2] >= '0' && buff[i + 2] <= '9' &&
                buff[i + 3] >= '0' && buff[i + 3] <= '9') {
                return (buff[i + 1] - '0') * 100 + (buff[i + 2] - '0') * 10 + (buff[i + 3] - '0');
            }
            return 0;
        }
    }
...
static int http_find_header(const uint8_t* buff, int nb, const char* name, char* val, int vmax) {
    ...
    while (p < end) {
        ...
        if (p + nlen + 1 <= end && strncasecmp(p, name, nlen) == 0 && p[nlen] == ':') {

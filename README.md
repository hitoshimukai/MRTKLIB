...
**Latest — v0.6.13:** enhanced PPP-RTK/VRS a-priori SPP seed — the per-epoch single-point fix that seeds the CLAS/VRS filter now uses C/N0 weighting + TDCP jump-reject by default, improving kinematic fix rate and the fixed-epoch error tail (PPC urban: mean fix +1.0 pp, p95 3.23 → 3.02 m) while leaving static positioning unchanged ([#196](https://github.com/h-shiono/MRTKLIB/pull/196)). Also: `mrtk convert` unified on obsdef frequency indexing — some RINEX obs-type columns reorder, no positioning impact ([#71](https://github.com/h-shiono/MRTKLIB/issues/71)).

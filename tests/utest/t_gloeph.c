...
static void dumpgeph(geph_t* geph, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        printf(
            "(%3d) sat=%2d frq=%2d svh=%d age=%d toe=%s %s "
            "pos=%13.3f %13.3f %13.3f vel=%9.3f %9.3f %9.3f "
            "acc=%10.3E %10.3E %10.3E taun=%12.5E gamn=%12.5E\n",
            i + 1, geph[i].sat, geph[i].frq, geph[i].svh, geph[i].age, s1, s2, geph[i].pos[0], geph[i].pos[1],
            geph[i].pos[2], geph[i].vel[0], geph[i].vel[1], geph[i].vel[2], geph[i].acc[0], geph[i].acc[1],
            geph[i].acc[2], geph[i].taun, geph[i].gamn);
    }
}
/* readrnx() */
void utest1(void) {
...
void utest5(void) {
    ...
    for (i = 0; i < MAXSAT; i++) {
        satno2id(i + 1, id);
        printf("%2d %-4s %8.3f %8.3f %8.3f\n", i + 1, id, nav.pcvs[i].off[0][0], nav.pcvs[i].off[0][1],
               nav.pcvs[i].off[0][2]);
    }
    printf("\n");

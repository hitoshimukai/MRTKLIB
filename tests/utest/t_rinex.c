...
static void dumpobs(obs_t* obs) {
    ...
    for (i = 0; i < obs->n; i++) {
        ...
        printf("%s : %2d %2d %13.3f %13.3f %13.3f %13.3f  %d %d\n", str, obs->data[i].sat, obs->data[i].rcv,
               obs->data[i].L[0], obs->data[i].L[1], obs->data[i].P[0], obs->data[i].P[1], obs->data[i].LLI[0],
               obs->data[i].LLI[1]);
...
static void dumpnav(nav_t* nav) {
    ...
    for (i = 0; i < nav->n; i++) {
        ...
        printf("%s : %2d    %s %s %3d %3d %2d\n", str, nav->eph[i].sat, s1, s2, nav->eph[i].iode, nav->eph[i].iodc,
               nav->eph[i].svh);
...
static rnxopt_t opt2 = {
    ...
    .name = {"OBSERVER90123456789012345678901", "AGENCY7890123456789012345678901"},
    .rec = {"RCV1567890123456789012345678901", "RCV2567890123456789012345678901", "RCV3567890123456789012345678901"},
    .ant = {"ANT1567890123456789012345678901", "ANT2567890123456789012345678901", "ANT3567890123456789012345678901"},

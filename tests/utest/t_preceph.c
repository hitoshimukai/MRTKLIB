...
static void dumpeph(peph_t* peph, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        printf("time=%s\n", s);
        for (j = 0; j < MAXSAT; j++) {
            printf("%03d: %14.3f %14.3f %14.3f : %5.3f %5.3f %5.3f\n", j + 1, peph[i].pos[j][0], peph[i].pos[j][1],
                   peph[i].pos[j][2], peph[i].std[j][0], peph[i].std[j][1], peph[i].std[j][2]);
        }
    }
}
...
void utest5(void) {
    ...
    for (i = 0; i < 86400 * 2; i += 30) {
        ...
        fprintf(fp, "%02d %6d %14.3f %14.3f %14.3f %14.3f %14.3f %14.3f %14.3f %14.3f\n", sat, i, rs1[0], rs1[1],
                rs1[2], dts1[0] * 1E9, rs2[0], rs2[1], rs2[2], dts2[0] * 1E9);
    }
    fclose(fp);
    printf("%s utest4 : OK\n", __FILE__);
}

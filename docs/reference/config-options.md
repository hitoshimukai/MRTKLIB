...
| `correction` | enum | PPP, PPP-RTK, VRS | Correction source (lowercase; parsing is case-sensitive). Decouples the augmentation source from `mode`. `none` · `igs` (precise SP3/CLK files) · `qzs-madoca` · `qzs-clas` · `igs-rts`† · `gal-has`† · `bds-b2b`† (†reserved/not yet implemented). Optional: when omitted it is inferred from `mode` + `satellite_ephemeris` for backward compatibility. See [Positioning Configuration Model](../design/configuration.md). **Note:** `igs` enables conventional IGS-products float PPP and works for geodetic receivers (e.g. GPS L1+L2, Galileo E1+E5a). Receivers that provide the 2nd frequency only as Galileo **E5b** / BeiDou **B2I** (no E5a/B3I, e.g. u-blox F9P) currently drop those constellations — see [#135](https://github.com/h-shiono/MRTKLIB/issues/135). |
